...
#define MAX_CHANNELS 8 // MCC 128 Channel Count
...
int main(void)
{
    ...
    app = gtk_application_new("mcc128.dataLogger", G_APPLICATION_FLAGS_NONE);
    ...
    if (retval == 0)
    {
        ...
        mcc128_a_in_scan_stop(g_hat_addr);
        // Clean up after the scan completes
        mcc128_a_in_scan_cleanup(g_hat_addr);
        // Close the device.
        mcc128_close(g_hat_addr);
    }
...
static gboolean allocate_channel_xy_arrays(int *channel)
{
    ...
    if (buff_size > 0)
    {
        graphChannelInfo[chan].graph = gtk_databox_lines_new((guint)buff_size, graphChannelInfo[chan].X,
                                           ...
                                           int num_chans)
{
    ...
    if (samples_per_chan_read > 0)
    {
        ...
        if ((samples_per_chan_displayed + samples_per_chan_read) <= display_buf_size_samples)
        {
        ...
        else if (samples_per_chan_read > display_buf_size_samples)
        {
            ...
            start_idx = ((samples_per_chan_read - display_buf_size_samples) * num_chans);
        ...
        else
        {
            ...
            start_idx = ((samples_per_chan_displayed - samples_to_keep) * num_chans);
...
static gboolean refresh_graph()
{
    ...
    start_sample = g_sample_count >= g_num_samples ? (g_sample_count - g_num_samples) : 0;
...
static void *read_and_display_data()
{
    ...
    while (g_done != TRUE)
    {
        ...
        retval = mcc128_a_in_scan_read(g_hat_addr, &read_status,
...
static void start_stop_event_handler(GtkWidget *widget, gpointer data)
{
    ...
    if (strcmp(StartStopBtnLbl, "Start") == 0)
    {
        ...
        retval = mcc128_a_in_scan_actual_rate(num_channels, g_sample_rate,
        ...
        retval = mcc128_a_in_scan_start(g_hat_addr, g_chan_mask,
    ...
    else
    {
        ...
        // Stop the scan
        retval = mcc128_a_in_scan_stop(g_hat_addr);
        if (retval != RESULT_SUCCESS)
        {
            show_error(&retval);
        }
        ...
        // Clean up after the scan completes
        retval = mcc128_a_in_scan_cleanup(g_hat_addr);
        if (retval != RESULT_SUCCESS)
        {
            show_error(&retval);
        }
...
// Find all of the MCC128 HAT devices that are installed
...
static int open_first_hat_device(uint8_t *hat_address)
{
    ...
    // Get the number of MCC 128 devices so that we can determine
    ...
    if (hat_count > 0)
    {
        ...
        hat_list(HAT_ID_MCC_128, hat_info_list);
        // Choose the first one
        *hat_address = hat_info_list[0].address;

        // Open the hat device
        retval = mcc128_open(*hat_address);
        if (retval != RESULT_SUCCESS)
        {
            show_error(&retval);
        }
